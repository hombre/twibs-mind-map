...
(function(n){class q{constructor(g){this.ideasMap=new Map;this.ideasByElement=new Map;this.ideasByShadow=new Map;this.ideas=()=>[...this.ideasMap.values()];this.load=a=>this.fromModels(JSON.parse(a));this.fromModels=a=>this.undoable(()=>a.forEach(a=>this.add(a)));this.save=()=>JSON.stringify(this.toModels());this.toModels=()=>this.ideas().map(a=>a.toModel());this.add=a=>{const b=new r(this);b.update(a);this.ideasMap.set(a.id,b);return b};this.remove=a=>{const b=this.ideasMap.get(a);b&&b.remove();
this.ideasMap.delete(a)};this.update=a=>this.ideasMap.get(a.id).update(a);this.createModel=(a,b,c="",e="",p=new d(0,0),l=!1,f=!1,h=!1)=>({id:a,parentId:b,title:c,key:e,x:p.x,y:p.y,selected:l,marked:f,collapsed:h});this.isMarking=this.isEditing=!0;this.onChanged=a=>{};this.undoActions=[];this.redoActions=[];this.ur=(a,b)=>({undo:a,redo:b});this.AddUR=a=>this.ur(()=>this.remove(a.id),()=>this.add(a));this.RemoveUR=a=>this.ur(()=>this.add(a),()=>this.remove(a.id));this.UpdateUR=(a,b)=>this.ur(()=>this.update(a),
...
a.ctrlKey){a.preventDefault();const b=this.scale*(1+(0<a.deltaY?-.1:.1));a=this.mouseToModelPos(a);a=a.divide(b/this.scale).minus(a);this.moveRootIdeas(a);this.updateScale(b)}};this.forceCtrlForWheel=!0;this.moveAround=a=>{const b=new t(this.mm,a);b.drag=a=>{this.moveRootIdeas(a.minus(b.lastPos).divide(this.scale));this.redraw(!1)};b.start()};this.moveIdea=(a,b)=>{const c=b.element.cloneNode(!0);c.classList.add("drag-image");const e=new t(this.mm,a);e.startDrag=()=>{b.element.classList.add("dragging");
this.mm.appendChild(c)};e.stopDrag=()=>{this.mm.removeChild(c);b.element.classList.remove("dragging")};e.drag=a=>b.elementPos().plus(a.minus(e.startPos).divide(this.scale)).toPosition(c);e.drop=a=>{this.undoable(()=>{function c(a){a.forEach(a=>a.pos=new d(-a.pos.x,a.pos.y))}var f=b.isLeft();const h=this.findIdeaByNode(a.target);if(h){var k=this.mouseToModelPos(a);(1==h.level()&&f!=h.absPos().x>k.x||1!=h.level()&&h.isLeft()!=f)&&c(b.thisAndDescendants());f=h.thisAndAncestors();k=f.indexOf(b);0<k&&
(f[k-1].parentId=b.parentId);b.parentId=h.id}else b.move((new d(a.clientX,a.clientY)).minus(e.startPos).divide(this.scale)),b.isLeft()!=f&&c(b.descendants())})};e.start()};this.toSvgPath=(a,b,c)=>this.toCurvedSvgPath(b,c);this.toCurvedSvgPath=(a,b)=>`M ${a.x} ${a.y} C ${a.x+(b.x-a.x)/1.5} ${a.y} ${b.x-(b.x-a.x)/2} ${b.y} ${b.x} ${b.y}`;this.container=g;this.container.tabIndex=0;this.mm=document.createElement("div");this.mm.className="tmm";this.container.appendChild(this.mm);this.svg=document.createElementNS("http://www.w3.org/2000/svg",
"svg");this.mm.appendChild(this.svg);this.shadow=document.createElement("div");this.shadow.className="tmm shadow";this.container.appendChild(this.shadow);g.addEventListener("mousedown",a=>{if(0==a.button&&!(a.ctrlKey||a.shiftKey||a.altKey||a.metaKey))if(a.target==this.mm)this.moveAround(a);else this.onIdeaDo(a.target,b=>this.moveIdea(a,b))});g.addEventListener("wheel",this.turnWheel);g.addEventListener("keydown",a=>{!(a.target!=this.container||a.shiftKey||a.ctrlKey||a.altKey||a.metaKey)&&0<=["ArrowUp",
"ArrowDown","ArrowLeft","ArrowRight"," "].indexOf(a.key)&&a.preventDefault()});g.addEventListener("keyup",a=>{if(!a.altKey&&!a.metaKey)if(a.ctrlKey)"z"==a.key&&this.undo(),"Z"==a.key&&this.redo()," "==a.key&&this.mark();else if("+"==a.key&&this.scaleUp(),"-"==a.key&&this.scaleDown(),"<"==a.key&&this.collapse(),">"==a.key&&this.expand(),"l"==a.key&&this.layout(!0),"f"==a.key&&this.fitToContainer(),"Delete"==a.key&&this.removeSelected(),"Enter"==a.key&&this.addSibling(),"Insert"==a.key&&this.addChild(),
!a.shiftKey&&this.isOneSelected()){const b=this.selected()[0];" "==a.key&&this.isEditing&&b.edit();"ArrowUp"==a.key&&b.arrowUp();"ArrowDown"==a.key&&b.arrowDown();"ArrowLeft"==a.key&&b.arrowLeft();"ArrowRight"==a.key&&b.arrowRight()}});g.addEventListener("dblclick",a=>this.onIdeaDo(a.target,a=>a.edit()));g.addEventListener("click",a=>{if(0==a.button)if(a.target==this.mm)this.deselectAll(),this.changed(!1);else this.onIdeaDo(a.target,b=>{a.ctrlKey?(b.selected=!b.selected,this.changed(!0)):b.selectOnly()})});
this.updateScale(parseFloat(window.localStorage.getItem("tmm.scale")||"1"));this.initResizeObserver()}}n.MindMap=q;class r{constructor(g){this.parentId=this.id=0;this.key=this.title="";this.pos=new d(0,0);this.collapsed=this.marked=this.selected=!1;this.dirty=!0;this.marginBottom=this.marginTop=this.marginRight=this.marginLeft=this.height=this.width=0;this.connectionOffsets=[];this.parent=()=>this.id==this.parentId?void 0:this.mindMap.ideasMap.get(this.parentId);this.level=()=>this.parent()?this.parent().level()+
1:1;this.isHidden=()=>this.parent()?this.parent().isHidden()||this.parent().isCollapsed():!1;this.isCollapsed=()=>this.collapsed;this.isLeft=()=>2==this.level()?0>this.pos.x:this.parent()?this.parent().isLeft():!1;this.canCollapse=()=>!this.collapsed&&0<this.children().length;this.canExpand=()=>this.collapsed&&0<this.children().length;this.children=()=>this.mindMap.ideas().filter(a=>a.parentId===this.id);this.thisAndAncestors=()=>1<this.level()?[this].concat(this.parent().thisAndAncestors()):[this];
this.thisAndDescendants=()=>{const a=[this];this.children().forEach(b=>a.push(...b.thisAndDescendants()));return a};this.descendants=()=>{const a=this.thisAndDescendants();a.shift();return a};this.absPos=()=>this.parent()?this.parent().absPos().plus(this.isHidden()?new d(0,0):this.pos):this.pos;this.elementPos=()=>this.mindMap.toDisplayPos(this.rect().leftTop().minus(new d(this.marginLeft,this.marginTop)));this.rect=()=>this.absPos().minus(new d(this.width/2,this.height/2)).toRect(this.width,this.height);
this.connectionPoints=()=>this.connectionOffsets.map(a=>this.absPos().plus(a));this.update=a=>{this.id=a.id;this.parentId=a.parentId;this.title=a.title;this.key=a.key;this.pos=new d(a.x,a.y);this.selected=a.selected;this.marked=a.marked;this.collapsed=a.collapsed};this.toModel=()=>({id:this.id,parentId:this.parentId,title:this.title,key:this.key,x:this.pos.x,y:this.pos.y,selected:this.selected,marked:this.marked,collapsed:this.collapsed});this.remove=()=>{this.mindMap.ideasByShadow.delete(this.shadow);
...
a.rect.top<b?e(a,b-a.rect.top):a.rect.bottom>c&&e(a,-(a.rect.bottom-c)),l=l.union(a.rect);else if(1<a.length){const k=a.shift(),f=a.pop(),p=k.rect.top<b?b:k.rect.top;k.rect.top<b&&e(k,b-k.rect.top);const m=f.rect.bottom>c?c:f.rect.bottom;f.rect.bottom>c&&e(f,-(f.rect.bottom-c));var h=m-p;if(d>h){b=p-b;c-=m;let n=-(d-h);h=d-h;0==b&&0==c?(n/=2,h/=2):(n*=b/(b+c),h*=c/(b+c));e(k,n);e(f,h);l=l.union(k.rect).union(f.rect);g(a,p+k.rect.height+n,m-f.rect.height+h,d-k.rect.height-f.rect.height)}else l=l.union(k.rect).union(f.rect),
g(a,p+k.rect.height,m-f.rect.height,d-k.rect.height-f.rect.height)}}let l=c?new m(this.rect().left,this.rect().top-this.marginTop,this.rect().width+this.marginRight,this.rect().height+this.marginTop):new m(this.rect().left-this.marginLeft,this.rect().top-this.marginTop,this.rect().width+this.marginLeft,this.rect().height+this.marginTop);if(!this.isCollapsed()&&0<b.length){b=b.map(b=>{const e=b.layout(a),f=c?a||this.rect().left<e.right?new d(this.rect().left-e.right,0):new d(0,0):a||this.rect().right>
...
r;const u=new Map;n.init=function(d){if(null==d)throw"Element must not be null";var a=u.get(d);if(a)return a;a=new q(d);u.set(d,a);return a};class t{constructor(g,a){this.dragging=!1;this.minDistance=4;this.keyUp=a=>{"Escape"==a.key&&this.stop()};this.mouseUp=a=>{this.stop();this.dragging&&this.drop(a)};this.click=a=>{this.dragging&&(a.preventDefault(),a.stopPropagation())};this.mouseMove=a=>{a=new d(a.clientX,a.clientY);const b=a.minus(this.startPos);!this.dragging&&b.length()>this.minDistance&&
